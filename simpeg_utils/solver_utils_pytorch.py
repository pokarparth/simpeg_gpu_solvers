...
#from discretize.utils.matrix_utils_cupy import mkvc
...
def SolverWrapD(fun, factorize=True, checkAccuracy=True, accuracyTol=1e-6, name=None):
    ...
    def __init__(self, A, **kwargs):
        ...
        self.kwargs = kwargs
        #t1 = time.time()
        #self.A_LU,self.piv_LU = torch.linalg.lu_factor(A,pivot=True)
        #t2 = time.time()
        #print('torch factorization time: ', t2 - t1)
    ...
    def __mul__(self, b):
        ...
        if len(b.shape) == 1 or b.shape[1] == 1:
            ...
            #X = np.empty_like(b)
            #t1 = time.time()
            #X = fun(self.A_LU, self.piv_LU, b, **self.kwargs)
            X = fun(self.A, b, **self.kwargs)
            #t2 = time.time()
            #print('torch solve time: ', t2 - t1)
            #for i in range(b.shape[1]):
                #if factorize:
                #    X[:, i] = self.solver.solve(b[:, i])

                #else:
                    #X[:, i] = fun(self.A_LU,self.piv_LU, b[:, i], **self.kwargs)
